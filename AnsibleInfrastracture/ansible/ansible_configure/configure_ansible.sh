...
# Author: Ostap Kravchuk
# Version: 0.1
# Date Created:           01st October 2018
# Last Modified:          08st October 17:26 GMT
# Last Modified By:       Ostap Kravchuk
# Description:
...
#  1 - i: IP Pattern
#  2 - n: Number of nodes
#  3 - f: filesystem : ext4 or xfs

# Note :
# This script has only been tested on Ubuntu 16.04 LTS
#########################################################
...
while getopts :i:n:f:a:k: optname; do
...
function install_ansible_ubuntu()
{
 ...
 function get_sshkeys()
 {
   ...
   python GetSSHFromPrivateStorageAccount.py ${AZ_ACCOUNT_NAME} ${AZ_ACCOUNT_KEY} id_rsa
   
   python GetSSHFromPrivateStorageAccount.py ${AZ_ACCOUNT_NAME} ${AZ_ACCOUNT_KEY} id_rsa.pub
}
...
function configure_ssh()
{
	...
	cp id_rsa /home/ansible/.ssh
	touch /home/ansible/.ssh/fast2016.pub
    mv id_rsa ~/.ssh
	...
	chmod 600 /home/ansible/.ssh/fast2016.pub
	chown ansible:ansible /home/ansible/.ssh/id_rsa
	chown ansible:ansible /home/ansible/.ssh/fast2016.pub
 ...
 function configure_ansible()
 {
	...
	# Add ansuble user to sudoers with no password
	echo -e "ansible ALL=(ALL:ALL) NOPASSWD: ALL" | sudo tee -a /etc/sudoers
	...
	#printf  "\n[master]\n10.200.26.76" >> "${ANSIBLE_HOST_FILE}"

	# Add group vars
	sudo mkdir -p /etc/ansible/group_vars
	#echo -e "---\nansible_ssh_user: root" | sudo tee -a /etc/ansible/group_vars/all    #I have done this becouse no ping permision root

    # Validate ansible configuration
    ansible -i /etc/ansible/hosts all -m ping -v
 }
...
function download_playbooks()
{
	# 1. Import Microsoft repo
	...
	# 5. Download playbooks from storage usind azcopy
	azcopy \
		--source https://${AZ_ACCOUNT_NAME}.file.core.windows.net/${FILE_SHARE_NAME}/ \
		--destination /home/ansible \
		--source-key ${AZ_ACCOUNT_KEY} \
		--recursive
	chown -R ansible:ansible /home/ansible/ansible_playbooks
}

function configure_inventory() # Make it readable from parameters
{
	eval "cat >/etc/ansible/hosts << EOF
# all
[all]
Client1 ansible_ssh_host=10.0.2.20  
Client2 ansible_ssh_host=10.0.2.21

EOF
"
}

function pre_deploy_asset()
{
	# Before deploy K8s
	...
	sudo pip install -r /home/ansible/ansible_playbooks/fast-data-deployment/requirements.txt
}

function deploy_asset()
{
	su ansible
	cd /home/ansible/ansible_playbooks/fast-data-deployment
